...
module.exports = t => {
  ...
  const createDependencyInjectionExpression = (
    dependencyNode,
    variableName
  ) => {
    if (
    ...
    if (variableName) {
      return t.variableDeclaration("var", [
        t.variableDeclarator(variableName, requireCall)
      ]);
    } else {
    ...
    return (
      ...
      dependencyList.elements.some(
        element =>
          ...
          (element.value === MODULE || element.value === EXPORTS)
      )
    );
  };
  ...
  const isSimplifiedCommonJSWrapperWithModuleOrExports = (
    dependencyList,
    factoryArity
  ) => {
    return (
      isSimplifiedCommonJSWrapper(dependencyList, factoryArity) &&
      factoryArity > 1
    );
  };
  ...
  const isFunctionExpression = factory => {
    return (
      t.isFunctionExpression(factory) || t.isArrowFunctionExpression(factory)
    );
  };

  const createFactoryReplacementExpression = (
    factory,
